...
void Scaling::teste(MatrixXd A) {
  ...
  cout << "min max ratio row: min " << compute_min_max_row_ratio().first
       << " max: " << compute_min_max_row_ratio().second << endl;
  cout << "min max ratio col: min " << compute_min_max_col_ratio().first
       << " max: " << compute_min_max_col_ratio().second << endl;
...
void Scaling::geometric_scale(MatrixXd &A, VectorXd &b, VectorXd &c,
                              VectorXd &l, VectorXd &u, int flag) {
  int m = A_abs.rows();
  int n = A.cols();
...
void Scaling::geometric_iterate(MatrixXd &A, VectorXd &b, VectorXd &c,
