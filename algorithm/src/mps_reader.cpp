    ...
    if (!firstWord.empty() && firstWord.find("*") != 0) {
      ...
      } else if (_checkSectionName(firstWord) == 2) // rows
      {
        ...
        while (_checkSectionName(firstWord) == -1) {
          ...
          if (firstWord.compare("L") == 0 || firstWord.compare("G") == 0 ||
              firstWord.compare("E") == 0) {
            n_rows++;
            n_rows_inq++;
      ...
      } else if (_checkSectionName(firstWord) == 3) // cols
      {
        ...
        while (_checkSectionName(firstWord) ==
               -1) // continue if the keyword is not a feild name
        {
  ...
  do {
    ...
    }
    {
      if (_checkSectionName(label) == 10) {
        std::cout
            << /*"Error: MPSREADER only accept LO and UP for Bounds"*/ std::
                   endl;
        break;
      }
    }
...
void mpsReader::_splitRaw(MatrixXd &Araw, VectorXd &braw, VectorXd &c,
  ...
  for (int i = 0; i < n_rows; i++) {
    if (row_labels[i] != "X") {
      ...
      if (row_labels[i] == "L") {
        ...
        // cout << "braw(i): " << braw(i) << " " << n_cols + counter_inq <<
        // endl;
        counter_inq++;
